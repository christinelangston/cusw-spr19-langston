...
void parseData(){
  ...
  county.outline = true;
  county.makeShape(); // county outline
  
  // now we can parse the population polygons
  int previd = 0;
  coords = new ArrayList<PVector>();
  for(int i =0; i < CensusBlocks.getRowCount(); i++){ // iterate thru table
    int shapeid = int(CensusBlocks.getString(i, 0));
    if(shapeid != previd){ // new polygon, create new polygon
     if(coords.size() >0){
  ...
  for(int i = 0; i < CensusPolygons.size(); i++){
   CensusPolygons.get(i).score = CensusData.getFloat(i, "B19113"); // correpsonds to code for metric 
