...
class Heatmap{
  ...
  float cellW, cellH;
  float[][] scores;
  color worst, mid, best; // add transition color
  PGraphics p;
  ...
  void draw(){ 
    ...
    for(int i = 0; i <cellX; i++){
     for(int j = 0; j <cellY; j++){
       ...
       float val = scores[i][j];
      if(val <50) col = lerpColor(worst, mid, (2*(val))/100);
      if(val== 50) col = mid;
      if(val > 50) col = lerpColor(mid, best, 2*((val /100)-.5));
      p.fill(col);
      p.noStroke();
